...
namespace tao{
/*LogEvent::LogEvent(const char* file, int32_t line, uint32_t elapse, uint32_t thread_id, uint32_t fiber_id, u_int64_t time)
    :m_file(file),
     m_line(line),
     m_elapse(elapse),
...
{
    if(level >= m_level){
        //std::cout<<"enter FileLogAppender"<<std::endl;
        MutexType::Lock lock(m_mutex);
        m_filestream << m_formatter->format(logger, level, event);
        //std::cout<<"out FileLogAppender"<<std::endl;
    }
}
